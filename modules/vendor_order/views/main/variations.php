<tr>
    ...
    <td <?= $item->id > 0 ? '' : 'class="td-hidden"'?> style="display: none">
    ...
    <td <?= $item->id > 0 ? '' : 'class="td-hidden"'?> style="width: 10rem;">
        <?= \yii\bootstrap5\Html::dropDownList('typeDetailId', $item->typeDetailId, $typesDetails, ['class' => 'form-select', 'prompt' => 'Выбрать']); ?>
    </td>
    <td <?= $item->id > 0 ? '' : 'class="td-hidden"'?>>
        <?php if ($item->id > 0) : ?>
